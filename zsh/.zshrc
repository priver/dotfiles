...
# ls
# --------------------------------------------------------------------------------------------------
if (( $+commands[dircolors] )) && command ls --version &>/dev/null; then
  # GNU
...
elif command grep --version &>/dev/null; then
...
if (( $+commands[fnm] )); then
  eval "$(fnm env --corepack-enabled --use-on-cd)" >/dev/null 2>&1
fi
