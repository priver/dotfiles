...
# prompt
PS1='${SSH_TTY:+"%(!.%F{red}%B.%F{242})%n%b%F{242}@%m:"}${SSH_TTY:-"%(!.%F{red}%B%n%b%F{242}:.)"}%F{blue}%B%~%b${(e)git_info[prompt]}${(e)git_info[status]:+" ${(e)git_info[status]}"}${(e)git_info[upstream]:+" ${(e)git_info[upstream]}"}${(e)git_info[stash]}${PRIVER_PROMPT_EXTRA}${VIRTUAL_ENV:+" %F{242}%{%G%} ${VIRTUAL_ENV:t}"}${duration_info}%(1j.%F{red} ✦.) %B%(?:%F{green}:%F{red})%{%G❯%}%f%b '
unset RPS1
